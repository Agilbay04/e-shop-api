...
type ProductRepository interface {
	...
	Delete(tx *gorm.DB, product *models.Product) error
...
func (r *productRepository) Delete(tx *gorm.DB, product *models.Product) error {
	if tx != nil {
		return tx.Save(product).Error
	}
	return r.db.Save(product).Error
...
func (r *productRepository) UpdateStock(tx *gorm.DB, id string, newStock int) error {
	return tx.Model(&models.Product{}).Where("id = ?", id).Update("stock", newStock).Error
}
